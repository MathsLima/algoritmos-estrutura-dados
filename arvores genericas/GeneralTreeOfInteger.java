...
public class GeneralTreeOfInteger {
    ...
    public boolean add(Integer elem, Integer elemFather) {
        ...
        if (elemFather == null) { // se insercao de elem como raiz
            if (root != null) { // se a arvore nao esta vazia
                root.parent = n; //pai da raiz antiga é o novo nodo
                n.addSubtree(root); //adiciona como filho do novo nodo a raiz antiga
            }
            root = n; //seta o novo elemento como raiz
            count++;
            return true;
        }
        else { //nodo inserido não será a raiz da arvore
    ...
    private void positionsPosAux(Node n, LinkedList<Integer> lista) {
        if( n != null ) {
            ...
            // Visita a raiz
            lista.add(n.element); //adiciona o elemento na list DEPOIS de visitar os filhos, POS-ORDEM
        }
    }    
